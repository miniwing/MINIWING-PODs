...
#import "UITableExcelView.h"
#import "UITableExcelCell.h"
#import "UITableExcelViewHeaderView.h"

@interface UITableExcelView ()<UITableViewDelegate,UITableViewDataSource, UITableExcelCellDelegate> {
...
/**当前控制样式的mode*/
@property (nonatomic, strong)             UITableExcelViewMode       * mode;
/** 内部通知的name */
@property (nonatomic, strong, readwrite)  NSString                   * notificationID;
@property (nonatomic, strong)             UITableView                * tableView;
@property (nonatomic, strong)             UITableExcelViewHeaderView * headView;
@property (nonatomic,   copy)             NSArray <UIColumnMode *>   * fixedColumnList;
@property (nonatomic,   copy)             NSArray <UIColumnMode *>   * slideColumnList;
@property (nonatomic, strong)             UIExcelCellConfig          * cellConfig;
@property (nonatomic, assign)             CGFloat                      cellLastX;
@property (nonatomic, strong)             UITableExcelCell           * excelCell;
...
@implementation UITableExcelView
...
- (void)tableView:(UITableView *)tableView
  willDisplayCell:(UITableExcelCell *)cell
      ...
      if (lastSelectedCell) {
         ...
         if (_lastColumn != -100) {
            ...
            if (_dataSourceHas.slideCellForRowAtIndexPathForMode) {
               ...
               UIColumnMode *model = slideList.count > _lastSelectedIndexPath.row ? slideList[_lastSelectedIndexPath.row]:nil;
               model.selected = NO;
            }
         }
      }
...
//MARK: ----------- init -----------
- (instancetype)initWithFrame:(CGRect)frame withMode:(UITableExcelViewMode *)mode {
...
- (void)prepareInit:(UITableExcelViewMode *)mode{
   
   //以当前对象的指针地址为通知的名称，这样可以避免同一个界面，有多个UIExcelView的对象时，引起的通知混乱
   _notificationID   = [NSString stringWithFormat:@"%p",self];
   _cellConfig    = [UIExcelCellConfig new];
   ...
   [_contentView addConstraint:NSLayoutAttributeLeft
                       equalTo:self
                        offset:0];
   
   [_contentView addConstraint:NSLayoutAttributeRight
                       equalTo:self
                        offset:0];
...
- (void)addVerticalView{
   ...
   if (_fixedColumnList.count > 0 && _addverticalDivider) {
      ...
      for (UIColumnMode *model in _fixedColumnList) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
   ...
   if ([scrollView isEqual:self.tableView]) {
      [[NSNotificationCenter defaultCenter] postNotificationName:_notificationID object:self userInfo:@{UI_EXCEL_NOTIFI_KEY:@(self.cellLastX)}];
   }
}
...
- (void)setDataSource:(id<UITableExcelViewDataSource>)dataSource{
...
- (void)setDelegate:(id<UITableExcelViewDelegate>)delegate {
   _delegate = delegate;
