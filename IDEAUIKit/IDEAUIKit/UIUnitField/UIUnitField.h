...
UIKIT_EXTERN NSNotificationName const UIUnitFieldDidBecomeFirstResponderNotification;
UIKIT_EXTERN NSNotificationName const UIUnitFieldDidResignFirstResponderNotification;
#else
UIKIT_EXTERN NSString *         const UIUnitFieldDidBecomeFirstResponderNotification;
UIKIT_EXTERN NSString *         const UIUnitFieldDidResignFirstResponderNotification;
#endif

/**
 UnitField 的外观风格
 
 - UIUnitFieldStyleBorder: 边框样式, UnitField 的默认样式
 - UIUnitFieldStyleUnderline: 下滑线样式
 */
typedef NS_ENUM(NSUInteger, UIUnitFieldStyle)
{
    UIUnitFieldStyleBorder,
    UIUnitFieldStyleUnderline
};

@protocol UIUnitFieldDelegate;

//IB_DESIGNABLE
@interface UIUnitField : UIControl <UITextInput>

@property (nullable, nonatomic, weak) id<UIUnitFieldDelegate> delegate;
...
/**
 允许输入的个数。
 目前 UIUnitField 允许的输入单元个数区间控制在 1 ~ 8 个。任何超过该范围内的赋值行为都将被忽略。
 */
@property (nonatomic, assign) IBInspectable NSUInteger inputUnitCount;

/**
 UnitField 的外观风格, 默认为 UIUnitFieldStyleBorder.
 */
@property (nonatomic, assign) IBInspectable NSUInteger style;
#else
@property (nonatomic, assign, readonly) NSUInteger inputUnitCount;
@property (nonatomic, assign, readonly) UIUnitFieldStyle style;
#endif

@property (nonatomic, assign) IBInspectable BOOL autoUpcase;
...
@protocol UIUnitFieldDelegate <UITextFieldDelegate>

@optional
- (BOOL)unitField:(UIUnitField *)uniField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string;
