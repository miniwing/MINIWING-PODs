...
@property (nonatomic,strong)IDEALanScanPing *simplePing;
...
-(instancetype)initWithIPToPing:(NSString*)ip andCompletionHandler:(nullable void (^)(NSError  * _Nullable error, NSString  * _Nonnull ip))result;{
    ...
    if (self) {
        self.name = ip;
        _ipStr= ip;
        _simplePing = [IDEALanScanPing simplePingWithHostName:ip];
...
- (void)simplePing:(IDEALanScanPing *)pinger didSendPacket:(NSData *)packet {
    //This timer will fired pingTimeOut in case the IDEALanScanPing don't answer in the specific time
