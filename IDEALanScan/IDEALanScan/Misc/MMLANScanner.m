...
- (instancetype)initWithDelegate:(id<MMLANScannerDelegate>)delegate {
   ...
   NSBundle    *stBaseBundle     = nil;
   NSString    *szBundlePath     = nil;
   ...
   if (self) {
      ...
      stBaseBundle   = [NSBundle bundleForClass:self.class];
      szBundlePath   = [stBaseBundle pathForResource:@"IDEALanScan" ofType:@"bundle"];
      stBundle       = [NSBundle bundleWithPath:szBundlePath];
      
      //Initializing the dictionary that holds the Brands name for each MAC Address
      _brandDictionary  = [NSDictionary dictionaryWithContentsOfFile:[stBundle pathForResource:@"BRAND" ofType:@"plist"]];
...
- (void)start {
   ...
   if (0 != self.queue.operationCount) {
   ...
   for (NSString *ipStr in self.ipsToPing) {
      ...
      MACOperation *macOperation = [[MACOperation alloc] initWithIPToRetrieveMAC:ipStr
                                                            ...
                                                            andCompletionHandler:^(NSError * _Nullable error, NSString * _Nonnull ip, MMDevice * _Nonnull device) {
