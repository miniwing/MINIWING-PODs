...
#import "IDEAGeigerCounter.h"
#import <AudioToolbox/AudioToolbox.h>

@interface IDEAGeigerCounter ()
...
@implementation IDEAGeigerCounter
...
- (void)start
{
    NSURL *tickSoundURL = [[NSBundle bundleForClass:IDEAGeigerCounter.class] URLForResource:@"IDEAGeigerCounterTick" withExtension:@"aiff"];
...
- (void)enable
{
    ...
    switch (self.position) {
        case IDEAGeigerCounterPositionLeft:
            xOrigin = 0.0;
            autoresizingMask |= UIViewAutoresizingFlexibleRightMargin;
            break;
        case IDEAGeigerCounterPositionMiddle:
            ...
            break;
        case IDEAGeigerCounterPositionRight:
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        _position = IDEAGeigerCounterPositionLeft;

        _meterPerfectColor = [IDEAGeigerCounter colorWithHex:0x999999 alpha:1.0];
        _meterGoodColor = [IDEAGeigerCounter colorWithHex:0x66a300 alpha:1.0];
        _meterBadColor = [IDEAGeigerCounter colorWithHex:0xff7f0d alpha:1.0];
...
{
    static IDEAGeigerCounter *instance;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[IDEAGeigerCounter alloc] init];
    });
    return instance;
}
