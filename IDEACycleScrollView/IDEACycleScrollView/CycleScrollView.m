...
@property (nonatomic, strong)                CycleScrollViewCell                 * currentCell;
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
   ...
   if ([self pointInside:point withEvent:event]) {
      ...
      for (UIView *cell in self.scrollView.subviews) {
         ...
         if (self.direction == CycleScrollViewScrollDirectionHorizontal) {
...
- (void)reloadData {
   ...
   for (NSInteger H = 0; H < self.showCount; H++) {
...
- (CycleScrollViewCell *)dequeueReusableCell {
   ...
   CycleScrollViewCell *cell = self.reusableCells.lastObject;
...
- (void)startTimer {
   ...
   if (self.realCount > 1 && self.isAutoScroll) {
      ...
      NSTimer *timer = [NSTimer scheduledTimerWithTimeInterval:self.autoScrollTime
                                                        target:self
   ...
   switch (self.direction) {
      case CycleScrollViewScrollDirectionHorizontal:{
         ...
         for (NSInteger i = self.visibleRange.location; i < NSMaxRange(self.visibleRange); i++) {
            CycleScrollViewCell *cell = self.visibleCells[i];
...
- (void)handleScrollViewDidScroll:(UIScrollView *)scrollView {
   ...
   if ([self.delegate respondsToSelector:@selector(cycleScrollView:scrollingFromIndex:toIndex:ratio:)]) {
      ...
      }else if (self.direction == CycleScrollViewScrollDirectionVertical) {
