//
//  IDEAPopControllerAnimationContext.m
//  IDEAPopController
//
...
@implementation IDEAPopAnimationContext
...
@interface IDEAPopControllerAnimatedTransitioning ()

@property (nonatomic, strong) id<IDEAPopControllerAnimationProtocol> animator;
...
@property (nonatomic, strong) IDEAPopAnimationContext *animationContext;

@end

@implementation IDEAPopControllerAnimatedTransitioning

- (instancetype)initWithState:(IDEAPopState)state popController:(IDEAPopController *)popController {
   self = [super init];
   if (self) {
      _state = state;
      ...
      _animationContext = [[IDEAPopAnimationContext alloc] initWithState:state containerView:_containerView];
...
- (void)getAnimator {
   if (self.popController.animationProtocol) {
      ...
      IDEADefaultPopAnimator *defaultPopAnimator = [IDEADefaultPopAnimator new];
...
- (void)animateTransition:(id <UIViewControllerContextTransitioning>)transitionContext {
   switch (self.state) {
      case IDEAPopStatePop: {
         [self popAnimateTransition:transitionContext];
      }
         break;
      case IDEAPopStateDismiss:{
