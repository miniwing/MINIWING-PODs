//
//  __CGUtilities.m
//  __Kit <https://github.com/ibireme/YYKit>
//
...
#import "__CGUtilities.h"

CGContextRef __CGContextCreateARGBBitmapContext(CGSize size, BOOL opaque, CGFloat scale) {
...
CGContextRef __CGContextCreateGrayBitmapContext(CGSize size, CGFloat scale) {
    size_t width = ceil(size.width * scale);
    size_t height = ceil(size.height * scale);
    if (width < 1 || height < 1) return NULL;
...
CGAffineTransform __CGAffineTransformGetFromPoints(CGPoint before[3], CGPoint after[3]) {
...
CGAffineTransform __CGAffineTransformGetFromViews(UIView *from, UIView *to) {
    ...
    return __CGAffineTransformGetFromPoints(before, after);
}
