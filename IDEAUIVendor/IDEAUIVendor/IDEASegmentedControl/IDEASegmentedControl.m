...
@interface IDEASegmentedControl ()
...
@implementation IDEASegmentedControl
...
- (id)initWithSectionImages:(NSArray<UIImage *> *)sectionImages sectionSelectedImages:(NSArray<UIImage *> *)sectionSelectedImages {
   ...
   if (self) {
      ...
      self.type = IDEASegmentedControlTypeImages;
   }
...
- (instancetype)initWithSectionImages:(NSArray<UIImage *> *)sectionImages sectionSelectedImages:(NSArray<UIImage *> *)sectionSelectedImages titlesForSections:(NSArray<NSString *> *)sectiontitles {
   ...
   if (self) {
      ...
      self.sectionSelectedImages = sectionSelectedImages;
      self.sectionTitles = sectiontitles;
      self.type = IDEASegmentedControlTypeTextImages;
   }
...
- (void)commonInit {
   ...
   self.selectionStyle = IDEASegmentedControlSelectionStyleTextWidthStripe;
   self.selectionIndicatorLocation = IDEASegmentedControlSelectionIndicatorLocationUp;
   self.segmentWidthStyle = IDEASegmentedControlSegmentWidthStyleFixed;
...
- (void)setSelectionIndicatorLocation:(IDEASegmentedControlSelectionIndicatorLocation)selectionIndicatorLocation {
   _selectionIndicatorLocation = selectionIndicatorLocation;
   
   if (selectionIndicatorLocation == IDEASegmentedControlSelectionIndicatorLocationNone) {
...
- (void)setSegmentWidthStyle:(IDEASegmentedControlSegmentWidthStyle)segmentWidthStyle {
   // Force IDEASegmentedControlSegmentWidthStyleFixed when type is IDEASegmentedControlTypeImages.
   if (self.type == IDEASegmentedControlTypeImages) {
      _segmentWidthStyle = IDEASegmentedControlSegmentWidthStyleFixed;
   } else {
      _segmentWidthStyle = segmentWidthStyle;
   }
}

- (void)setBorderType:(IDEASegmentedControlBorderType)borderType {
...
- (void)drawRect:(CGRect)rect {
   ...
   if (self.type == IDEASegmentedControlTypeText) {
      [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
         ...
         BOOL locationUp = (self.selectionIndicatorLocation == IDEASegmentedControlSelectionIndicatorLocationUp);
         BOOL selectionStyleNotBox = (self.selectionStyle != IDEASegmentedControlSelectionStyleBox);
         ...
         //           CGFloat y = roundf((CGRectGetHeight(self.frame) - selectionStyleNotBox * self.selectionIndicatorHeight) - stringHeight + self.selectionIndicatorHeight * locationUp);
         
         CGRect rect;
         if (self.segmentWidthStyle == IDEASegmentedControlSegmentWidthStyleFixed) {
   ...
   } else if (self.type == IDEASegmentedControlTypeImages) {
      [self.sectionImages enumerateObjectsUsingBlock:^(id iconImage, NSUInteger idx, BOOL *stop) {
         UIImage *icon = iconImage;
         CGFloat imageWidth = icon.size.width;
         CGFloat imageHeight = icon.size.height;
         CGFloat y = roundf(CGRectGetHeight(self.frame) - self.selectionIndicatorHeight) / 2 - imageHeight / 2 + ((self.selectionIndicatorLocation == IDEASegmentedControlSelectionIndicatorLocationUp) ? self.selectionIndicatorHeight : 0);
   ...
   } else if (self.type == IDEASegmentedControlTypeTextImages){
      [self.sectionImages enumerateObjectsUsingBlock:^(id iconImage, NSUInteger idx, BOOL *stop) {
         UIImage *icon = iconImage;
         CGFloat imageWidth = icon.size.width;
         CGFloat imageHeight = icon.size.height;
         ...
         if (self.segmentWidthStyle == IDEASegmentedControlSegmentWidthStyleFixed) {
            BOOL isImageInLineWidthText = self.imagePosition == IDEASegmentedControlImagePositionLeftOfText || self.imagePosition == IDEASegmentedControlImagePositionRightOfText;
            if (isImageInLineWidthText) {
               CGFloat whitespace = self.segmentWidth - stringSize.width - imageWidth - self.textImageSpacing;
               if (self.imagePosition == IDEASegmentedControlImagePositionLeftOfText) {
               ...
               if (self.imagePosition == IDEASegmentedControlImagePositionAboveText) {
                  imageYOffset = ceilf(whitespace / 2.0);
                  textYOffset = imageYOffset + imageHeight + self.textImageSpacing;
               } else if (self.imagePosition == IDEASegmentedControlImagePositionBelowText) {
                  textYOffset = ceilf(whitespace / 2.0);
                  imageYOffset = textYOffset + stringHeight + self.textImageSpacing;
               }
            }
         } else if (self.segmentWidthStyle == IDEASegmentedControlSegmentWidthStyleDynamic) {
            // When we are drawing dynamic widths, we need to loop the widths array to calculate the xOffset
            CGFloat xOffset = 0;
            NSInteger i = 0;
   ...
   if (self.selectedSegmentIndex != IDEASegmentedControlNoSegment) {
      if (self.selectionStyle == IDEASegmentedControlSelectionStyleArrow) {
         ...
         if (!self.selectionIndicatorStripLayer.superlayer) {
            ...
            if (self.selectionStyle == IDEASegmentedControlSelectionStyleBox && !self.selectionIndicatorBoxLayer.superlayer) {
...
- (void)addBackgroundAndBorderLayerWithRect:(CGRect)fullRect {
   ...
   // Border layer
   if (self.borderType & IDEASegmentedControlBorderTypeTop) {
...
- (void)setArrowFrame {
   ...
   if (self.selectionIndicatorLocation == IDEASegmentedControlSelectionIndicatorLocationDown) {
   ...
   if (self.selectionIndicatorLocation == IDEASegmentedControlSelectionIndicatorLocationUp) {
...
- (CGRect)frameForSelectionIndicator {
   ...
   if (self.selectionStyle == IDEASegmentedControlSelectionStyleArrow) {
      ...
      if (self.selectionStyle == IDEASegmentedControlSelectionStyleTextWidthStripe &&
          sectionWidth <= self.segmentWidth &&
          self.segmentWidthStyle != IDEASegmentedControlSegmentWidthStyleDynamic) {
         CGFloat widthToEndOfSelectedSegment = (self.segmentWidth * self.selectedSegmentIndex) + self.segmentWidth;
         CGFloat widthToStartOfSelectedIndex = (self.segmentWidth * self.selectedSegmentIndex);
         ...
         if (self.segmentWidthStyle == IDEASegmentedControlSegmentWidthStyleDynamic) {
...
- (void)updateSegmentsRects {
   ...
   if (self.type == IDEASegmentedControlTypeText && self.segmentWidthStyle == IDEASegmentedControlSegmentWidthStyleFixed) {
      [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
         CGFloat stringWidth = [self measureTitleAtIndex:idx].width + self.segmentEdgeInset.left + self.segmentEdgeInset.right;
         self.segmentWidth = MAX(stringWidth, self.segmentWidth);
      }];
   } else if (self.type == IDEASegmentedControlTypeText && self.segmentWidthStyle == IDEASegmentedControlSegmentWidthStyleDynamic) {
   ...
   } else if (self.type == IDEASegmentedControlTypeTextImages && self.segmentWidthStyle == IDEASegmentedControlSegmentWidthStyleFixed){
      //lets just use the title.. we will assume it is wider then images...
      [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
         CGFloat stringWidth = [self measureTitleAtIndex:idx].width + self.segmentEdgeInset.left + self.segmentEdgeInset.right;
         self.segmentWidth = MAX(stringWidth, self.segmentWidth);
      }];
   } else if (self.type == IDEASegmentedControlTypeTextImages && self.segmentWidthStyle == IDEASegmentedControlSegmentWidthStyleDynamic) {
      NSMutableArray *mutableSegmentWidths = [NSMutableArray array];
      __block CGFloat totalWidth = 0.0;
      ...
      [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
         ...
         if (self.imagePosition == IDEASegmentedControlImagePositionLeftOfText || self.imagePosition == IDEASegmentedControlImagePositionRightOfText) {
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
   ...
   if (CGRectContainsPoint(enlargeRect, touchLocation)) {
      ...
      } else if (self.type == IDEASegmentedControlTypeTextImages || self.type == IDEASegmentedControlTypeText) {
...
- (void)setSelectedSegmentIndex:(NSUInteger)index animated:(BOOL)animated notify:(BOOL)notify {
   ...
   if (index == IDEASegmentedControlNoSegment) {
      ...
      if (animated) {
         ...
         if(self.selectionStyle == IDEASegmentedControlSelectionStyleArrow) {
            ...
            if ([self.selectionIndicatorStripLayer superlayer] == nil) {
               ...
               if (self.selectionStyle == IDEASegmentedControlSelectionStyleBox && [self.selectionIndicatorBoxLayer superlayer] == nil)
                  [self.scrollView.layer insertSublayer:self.selectionIndicatorBoxLayer atIndex:0];
